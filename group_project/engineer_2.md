...
-Github: https://github.com/nonnerdling/DeepHINT 

-Outcome: partial success

-Process:  I cloned the repository, created a dedicated Python 3.6 conda environment, and installed the legacy dependencies required by the codebase, including bedtools, Keras 2.0.8, TensorFlow 1.15, SciPy, scikit-learn, and libgfortran. After downloading and indexing the hg19 reference genome from UCSC, I modified the preprocessing scripts to be Python 3–compatible (e.g., fixing Python 2 print statements) and successfully generated the FASTA files and NumPy test matrices. I then resolved multiple dependency and runtime issues related to deprecated Keras APIs and binary incompatibilities, after which I was able to run the provided ensemble testing code using the pretrained model weights. This produced valid AUROC and AUPR scores and confirmed that the base DeepHINT pipeline executed on my machine, but with slightly lower performance than reported in the paper, which could be due to environmental and preprocessing differences. I was not able to fully reproduce the DeepHINT + H3K36me3 model described in the paper: while I downloaded the ENCODE H3K36me3 ChIP-seq bigWig file, attempted to extract signal features, and implemented a modified multi-input network to incorporate ChIP-seq data, the repository does not provide pretrained weights, a complete training script, or a fully specified preprocessing pipeline for the epigenomic features. As a result, although I made substantial progress integrating the data at a code level, I could not train or evaluate the H3K36me3-enhanced model in a way that reproduces the paper’s reported performance.

-Engineer: Rayhan Zaman 

Report #6

-Github: https://github.com/sigven/oncoEnrichR 

-Outcome: success/fail

-Process: installed the package along with its required dependencies. The software could be executed as intended, and the core enrichment functionality ran without errors on example inputs, confirming that the package itself is functional. However, the repository is primarily designed as a general-purpose analysis tool rather than a fully specified reproducibility pipeline, and it does not include a reference dataset, fixed parameters, or expected outputs tied directly to the results reported in the associated publication.
